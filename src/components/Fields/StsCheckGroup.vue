<template>
    ...
    <div class="field" v-if="showField">
        <fieldset>
            ...
            <div :class="{'cmn-wrapper': true, 'is-danger': hasError }">
                <div class="field" role="group">
                    <div class="control" v-for="(option, index) in field.options" :key="index" v-if="!option.renderCondition || option.renderCondition && evalCondition(option.conditionalModel, option.renderCondition)">
                        <label class="checkbox spaced">
                            <input :key="'field-' + _uid + '.' + index" type="checkbox" :name="field.name" v-model="fieldModels[field.model]" :value="option.value">
                            {{ option.label }}
                        </label>
                    </div>
                </div>
